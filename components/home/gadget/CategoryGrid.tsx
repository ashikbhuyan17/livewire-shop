...
import type { HeaderCategoryItem } from '@/fetch/buildCategoryMenu';
...
export default function CategoryGrid({ categories }: Props) {
  ...
  const showSeeAll = categories.length > HOME_CATEGORY_LIMIT;
  const visibleCategories = showSeeAll
    ...
    >
      ...
      <div className="grid grid-cols-2 gap-2.5 min-[480px]:grid-cols-3 sm:grid-cols-4 sm:gap-3 md:gap-3.5 lg:grid-cols-8 lg:gap-4">
        {visibleCategories.map((category) => {
          const imageSrc = resolveApiImage(category.icon);
          return (
            <Link
              key={category.slug}
              href={category.slug}
              className="group relative flex flex-col overflow-hidden rounded-xl bg-[#f3f4f6] pb-3 pt-1 transition-all duration-300 hover:bg-white hover:shadow-[0_8px_24px_rgba(37,99,235,0.12)] hover:ring-2 hover:ring-primary/25 sm:rounded-2xl sm:pb-3.5"
            >
              <div className="relative flex h-[5.25rem] items-end justify-center px-2 sm:h-[5.75rem] md:h-[6.25rem] lg:h-[5.5rem] xl:h-[6rem]">
                ...
                {imageSrc ? (
                  <Image
                    ...
                    width={140}
                    height={140}
                    className="relative z-10 max-h-[4.25rem] w-auto max-w-[85%] object-contain drop-shadow-[0_10px_14px_rgba(37,99,235,0.18)] transition-all duration-300 group-hover:-translate-y-2 group-hover:scale-105 group-hover:drop-shadow-[0_16px_22px_rgba(37,99,235,0.28)] sm:max-h-[4.75rem] md:max-h-[5.25rem] lg:max-h-[4.5rem] xl:max-h-[5rem]"
                  />
                ) : null}
              </div>
